...
export async function GET() {
    ...
    const stream = new ReadableStream({
        async start(controller) {
            try {
                controller.enqueue(encoder.encode(`data: {"message":"Initializing Shopify CLI...","step":0,"progress":0}\n\n`));
                
                // Verify Shopify CLI is accessible
                try {
                    execSync(`"${shopifyCLIPath}" version`, { 
                        stdio: 'pipe',
                        shell: true 
                    });
                } catch (cliError) {
                    throw new Error(`Shopify CLI verification failed: ${cliError.message}`);
                }
